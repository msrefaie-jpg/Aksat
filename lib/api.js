...
// تُستدعى من المُشغّل المجدول (Cron): تذكير يومي بالأقساط المستحقّة/المتأخّرة
export async function runDailyReminders(env) {
  ...
  for (const uk of Object.keys(byUser)) {
    ...
    const msg = buildReminder(st[0].state);
    if (!msg) continue;
...
function buildReminder(state) {
  ...
  units.forEach(u => (u.installments || []).forEach(i => {
    ...
    if (days < 0) { overdue++; sum += Number(i.amount) || 0; }
    else if (days <= 7) { soon++; sum += Number(i.amount) || 0; }
  }));
  if (!overdue && !soon) return null;
  const parts = [];
  if (overdue) parts.push(`${overdue} قسط متأخّر`);
  if (soon) parts.push(`${soon} مستحق خلال أسبوع`);

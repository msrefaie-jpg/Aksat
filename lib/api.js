...
export async function runDailyReminders(env) {
  ...
  for (const uk of Object.keys(byUser)) {
    ...
    // الأحداث الجديدة = قسط أصبح ملحوظاً أو تغيّرت حالته منذ آخر تنبيه
    const newOverdue = [], newSoon = [];
    const current = {};
    for (const [id, info] of notable) {
      current[id] = info.status;
      if (prev[id] !== info.status) {
        (info.status === 'overdue' ? newOverdue : newSoon).push(info);
      }
    }
    ...
    if (!newOverdue.length && !newSoon.length) continue; // لا جديد → لا إشعار
    const msg = buildChangeMsg(newOverdue, newSoon);
...
// الأقساط الملحوظة حالياً: متأخّرة، أو مستحقّة خلال ٧ أيام
function computeNotable(state) {
  ...
  units.forEach(u => (u.installments || []).forEach(i => {
    ...
    else if (days <= WINDOW) status = 'soon';
    if (status) map.set(i.id, { status, days, amount: Number(i.amount) || 0, unit: u.name || '' });
  }));
  return map;
}
function buildChangeMsg(newOverdue, newSoon) {
  const total = newOverdue.length + newSoon.length;
  if (total === 1) {
    const x = newOverdue[0] || newSoon[0];
    const when = x.status === 'overdue'
      ? `متأخّر ${Math.abs(x.days)} يوم`
      : (x.days === 0 ? 'يستحق اليوم' : `يستحق خلال ${x.days} يوم`);
    return { title: 'أقساط — تذكير', body: `${x.unit}: ${Math.round(x.amount).toLocaleString('en-US')} £ — ${when}`, url: './' };
  }
  let sum = 0;
  [...newOverdue, ...newSoon].forEach(x => sum += x.amount);
  const parts = [];
  if (newOverdue.length) parts.push(`${newOverdue.length} قسط أصبح متأخّراً`);
  if (newSoon.length) parts.push(`${newSoon.length} قسط اقترب استحقاقه`);
  return { title: 'أقساط — تذكير', body: `${parts.join(' · ')} — الإجمالي ${Math.round(sum).toLocaleString('en-US')} £`, url: './' };
}

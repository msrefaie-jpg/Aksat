...
export async function ensureSchema(sql) {
  ...
  await sql`CREATE INDEX IF NOT EXISTS push_subs_user ON push_subs (user_key)`;
  schemaReady = true;
}

// مفاتيح VAPID ذاتية التوليد (تُخزَّن مرة واحدة في قاعدة البيانات)
export async function getVapid(sql) {
  const rows = await sql`SELECT value FROM app_config WHERE key = 'vapid'`;
  if (rows.length) return rows[0].value;
  const keys = await generateVapidKeys();
  await sql`INSERT INTO app_config (key, value) VALUES ('vapid', ${JSON.stringify(keys)}::jsonb)
    ON CONFLICT (key) DO NOTHING`;
  const again = await sql`SELECT value FROM app_config WHERE key = 'vapid'`;
